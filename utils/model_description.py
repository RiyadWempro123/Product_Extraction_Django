...
SECTION_SYNONYMS = {
    ...
    "Diaphragm / O-Ring Material": [
        "diaphragm oring material",  # fixed normalized form
        "diaphragm material"
    ],
    "Revision": ["revision"],
...
def clean_description(desc):
    # Remove any stray () or non-printable symbols
    desc = re.sub(r"\(.*?\)", "", desc)
    desc = desc.strip()
    return desc
...
def extract_model_description_chart(pdf_path, modelDescriptionChartPage):
